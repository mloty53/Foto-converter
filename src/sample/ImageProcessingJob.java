...
public class ImageProcessingJob extends Task<Void> {
    ...
    protected Void call() {
        if (targetDirectory == null) {
            updateMessage("Warning! Specify destination directory!");
            return null;
        } else if (sourceFile == null) {
            updateMessage("Warning! Choose files to convert!");
            return null;
        }
        updateMessage("Progressing...");
        updateProgress(0, 1);
        try {
            ...
            Path outputPath = Paths.get(targetDirectory.getAbsolutePath(), sourceFile.getName());
            ImageIO.write(greyscale, "jpg", outputPath.toFile());
        } catch (IOException ex) {
            updateMessage("Error! Conversion failed!");
